...
Additionally, several Atlas commands which have unclear documentation, discrepancies between their documentation and
implementation, or incorrect implementation now have the following behaviour:
...
- the formula used by the "GB" AddressType for converting ROM addresses into RAM addresses was incorrect;
	additionally, after calculating the 16-bit RAM address, it includes the bank number in bits 17+,
	e.g. ROM 0x123456 = RAM $7456 in bank 0x48, so if you write a 16-bit pointer you get $7456,
	but if you write a 24-bit pointer, you get $487456 instead
DONE
}
...
sub write_pointer {
	...
	if ($addressType eq 'LOROM00' || $addressType eq 'LOROM80') {
		...
		if ($addressType eq 'LOROM80') {
			$value += 0x800000;
		}
	} elsif ($addressType eq 'HIROM') {
		$value += 0xC00000;
	} elsif ($addressType eq 'GB') {
		# Atlas error: GB MBCs fix ROM 0x0000 - 0x3FFF to RAM $0000 - $3FFF, and all other ROM banks are swapped into RAM $4000 - $7FFF
		my $bank = int($value / 0x4000);
		$value = $value % 0x4000;
		if ($bank) {
			$value += 0x4000 + $bank * 0x10000;
		}
